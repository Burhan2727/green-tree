...
const displayCategories = (categories) => {
  ...
  for (const categori of categories) {
    ...
    btnDiv.innerHTML = `
    <a id="btn-active-${categori.id}" class=" hover:bg-[#15803D] hover:text-white md:w-full text-[#1F2937] cursor-pointer inline-block md:block rounded-sm p-3">${categori.category_name}
    </a>
    `;
